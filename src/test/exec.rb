...
class Exec < Test::Unit::TestCase
  ...
  def test_exec_uses_correct_path
	...
	test.expected_minimum_run_time = 3
    assert(test.run(), test.error_message() + " (If failing first make sure rsipp.pl is in the path and the rsipp.config.xml file is set up correctly and SIPP or RSIPP environment variable points to folder containing sipp.dtd!)")
  end
end
