...
public:
  ...
  bool isClientTransaction() const { return client; }
  bool isServerTransaction() const { return !client; }
  ...
  string getCseqMethod() const { return cseqMethod; }

  void setAckBranch(const string &newAckBranch) { ackBranch = newAckBranch; }
  const string &getAckBranch() const { return ackBranch; }

  void setLastReceivedMessage(const string &msg) { lastReceivedMessage = msg; }
  const string &getLastReceivedMessage() const { return lastReceivedMessage; }

  void setLastResponseCode(int responseCode) { lastResponseCode = responseCode; }
  int getLastResponseCode() { return lastResponseCode; }
  bool isLastResponseCode2xx() { return ((lastResponseCode >= 200) && (lastResponseCode <= 299)); }

  void setAckIndex(int newAckIndex) { ackIndex = newAckIndex; }
  int getAckIndex() { return ackIndex; }

  void setTransactionHash(unsigned long hash) { transactionResponseHash = hash; }
  unsigned long getTransactionHash() { return transactionResponseHash; }
